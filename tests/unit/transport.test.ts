...
import { installDom, setFormDescription, restoreGlobals, MiniScriptElement } from './test-dom'
...
describe('transport', () => {
  it('posts feedback submissions through the API route prefix', async () => {
    ...
    const { ObviousFeedback } = await import(`../../src/index?transport-prefix=${Date.now()}`)
    ObviousFeedback.init({ publicKey: 'fsk_pub_test', apiBaseUrl: 'https://pr-123.preview.obvious.dev' })
    const host = body.children[1]
    host.shadowRoot?.querySelector('.obv-trigger')?.dispatch('click', { preventDefault() {} } as Event)
    const restoreForm = setFormDescription('Route prefix test')
    host.shadowRoot?.querySelector('[data-submit-round="true"]')?.dispatch('click', {} as Event)
  ...
  it('does not duplicate the API route prefix when callers include it in apiBaseUrl', async () => {
    ...
    expect(source).toContain("normalizedBaseUrl.endsWith(API_ROUTE_PREFIX) ? '' : API_ROUTE_PREFIX")
  })
